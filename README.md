...
判停策略（页面「判停灵敏度」滑杆 1~10，或在 `speech/config.json` 调整）：
- 灵敏度越大越保守：加大判停静音时长(60→600ms)、提高完成阈值(0.50→0.90)、拉长未说完等待(1.5→5s)；
- 短片段（首句分句/语气词）不再立刻提交，先缓冲确认没有续话；
- 智能小队：Silero 检出停顿后，**SmartTurn v3.2 判断"你说完了吗"**——没说完就压住等你续话，说完才转写；
  每次决策会在 `speech/speech.log` 记录（prob=… -> commit/hold），`GET /api/speech/status` 的
  `stat`/`last_decision` 也能看到判停模型在运行与当前参数。
- **如果仍觉得面试官抢话** → 把滑杆往右调到 8~10（或 `config.json` 提高 `smart_turn.max_wait_ms`）。
...
| 职位信息 | 公司/职位/JD/考察重点，开始后锁定；刷新页面可恢复未结束的面试 |
